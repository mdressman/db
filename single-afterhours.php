		...
		<div class="row">		
			<div class="span6">
			    ...
			    <?php 
					...
					$afterhours_fields = array('_afterhours_date', '_afterhours_tickets_link','_afterhours_facebook_link','_afterhours_crateplayer_link','_afterhours_soldout');

					$afterhours_data = array();

					foreach($afterhours_fields as $afterhours_field) {
						$afterhours_data[$afterhours_field] = get_post_meta ($post->ID, $afterhours_field, true);
					}
				...
				?>
					...
					<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
						...
						<?php 
							// $terms = get_the_terms($post->ID, 'afterhours_cat'); 
							// $category_name = array();
							// $category_slug = array();
							...
							switch($afterhours_data['_afterhours_date']) {
								case "wednesday":
						...
						<?php if($afterhours_data['_afterhours_date']) { ?>
				...
				<div class="row">
					<div class="span3">
						...
						$connected_artists = new WP_Query( 
												array(
													'connected_type' => 'artists_to_afterhours',
					...
					<div class="span3">
						...
						<?php if($afterhours_data['_afterhours_tickets_link']) { ?>
						<div class="row">
							<div class="span3">
								<?php if($afterhours_data['_afterhours_soldout']) { ?>
								...
								<?php } else { ?>
									<a class="sidebar-button" href="<?php echo $afterhours_data['_afterhours_tickets_link'] ?>" target="_blank"><h1>BUY TICKETS</h1></a>
								<?php } ?>		
							</div>
						</div>
						<?php } ?>

						<?php if($afterhours_data['_afterhours_facebook_link']) { ?>
						<div class="row">
							<div class="span3">
								<a class="sidebar-button" href="<?php echo $afterhours_data['_afterhours_facebook_link'] ?>" target="_blank"><h1>FACEBOOK RSVP</h1></a>
							</div>
						</div>
						<? } ?>

						<?php if($afterhours_data['_afterhours_crateplayer_link']) { ?>
						<div class="row">
							<div class="span3">
								<a class="sidebar-button" href="<?php echo $afterhours_data['_afterhours_crateplayer_link'] ?>" target="_blank"><h1>Listen on CratePlayer</h1></a>
							</div>
						</div>
						<? } ?>
						...
						$connected_venue = new WP_Query( 
												array(
													'connected_type' => 'afterhours_to_venue',
													'connected_items' => get_queried_object(),
													'nopaging' => true,
												) 
											);
