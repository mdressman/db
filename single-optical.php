		...
		<div class="row">		
			<div class="span6">
			    ...
			    <?php 
					...
					$optical_fields = array('_optical_date', '_optical_tickets_link','_optical_facebook_link','_optical_crateplayer_link','_optical_soldout');

					$optical_data = array();

					foreach($optical_fields as $optical_field) {
						$optical_data[$optical_field] = get_post_meta ($post->ID, $optical_field, true);
					}
				...
				?>
					...
					<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
						...
						<?php 
							// $terms = get_the_terms($post->ID, 'optical_cat'); 
							// $category_name = array();
							// $category_slug = array();
							...
							switch($optical_data['_optical_date']) {
								case "wednesday":
						...
						<?php if($optical_data['_optical_date']) { ?>
				...
				<div class="row">
					<div class="span3">
						...
						$connected_artists = new WP_Query( 
												array(
													'connected_type' => 'artists_to_optical',
					...
					<div class="span3">
						...
						<?php if($optical_data['_optical_tickets_link']) { ?>
						<div class="row">
							<div class="span3">
								<?php if($optical_data['_optical_soldout']) { ?>
								...
								<?php } else { ?>
									<a class="sidebar-button" href="<?php echo $optical_data['_optical_tickets_link'] ?>" target="_blank"><h1>BUY TICKETS</h1></a>
								<?php } ?>	
							</div>
						</div>
						<?php } ?>

						<?php if($optical_data['_optical_facebook_link']) { ?>
						<div class="row">
							<div class="span3">
								<a class="sidebar-button" href="<?php echo $optical_data['_optical_facebook_link'] ?>" target="_blank"><h1>FACEBOOK RSVP</h1></a>
							</div>
						</div>
						<? } ?>

						<?php if($optical_data['_optical_crateplayer_link']) { ?>
						<div class="row">
							<div class="span3">
								<a class="sidebar-button" href="<?php echo $optical_data['_optical_crateplayer_link'] ?>" target="_blank"><h1>Listen on CratePlayer</h1></a>
							</div>
						</div>
						<? } ?>
						...
						$connected_venue = new WP_Query( 
												array(
													'connected_type' => 'optical_to_venue',
													'connected_items' => get_queried_object(),
													'nopaging' => true,
												) 
											);
