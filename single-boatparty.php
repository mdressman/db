		...
		<div class="row">		
			<div class="span6">
			    ...
			    <?php 
					...
					$boatparty_fields = array('_boatparty_date', '_boatparty_tickets_link','_boatparty_facebook_link','_boatparty_crateplayer_link','_boatparty_soldout');

					$boatparty_data = array();

					foreach($boatparty_fields as $boatparty_field) {
						$boatparty_data[$boatparty_field] = get_post_meta ($post->ID, $boatparty_field, true);
					}
				...
				?>
					...
					<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
						...
						<?php 
							// $terms = get_the_terms($post->ID, 'boatparty_cat'); 
							// $category_name = array();
							// $category_slug = array();
							...
							switch($boatparty_data['_boatparty_date']) {
								case "wednesday":
						...
						<?php if($boatparty_data['_boatparty_date']) { ?>
				...
				<div class="row">
					<div class="span3">
						...
						$connected_artists = new WP_Query( 
												array(
													'connected_type' => 'artists_to_boatparty',
					...
					<div class="span3">
						...
						<?php if($boatparty_data['_boatparty_tickets_link']) { ?>
						<div class="row">
							<div class="span3">
								<?php if($boatparty_data['_boatparty_soldout']) { ?>
								...
								<?php } else { ?>
									<a class="sidebar-button" href="<?php echo $boatparty_data['_boatparty_tickets_link'] ?>" target="_blank"><h1>BUY TICKETS</h1></a>
								<?php } ?>		
							</div>
						</div>
						<?php } ?>

						<?php if($boatparty_data['_boatparty_facebook_link']) { ?>
						<div class="row">
							<div class="span3">
								<a class="sidebar-button" href="<?php echo $boatparty_data['_boatparty_facebook_link'] ?>" target="_blank"><h1>FACEBOOK RSVP</h1></a>
							</div>
						</div>
						<? } ?>

						<?php if($boatparty_data['_boatparty_crateplayer_link']) { ?>
						<div class="row">
							<div class="span3">
								<a class="sidebar-button" href="<?php echo $boatparty_data['_boatparty_crateplayer_link'] ?>" target="_blank"><h1>Listen on CratePlayer</h1></a>
							</div>
						</div>
						<? } ?>
						...
						$connected_venue = new WP_Query( 
												array(
													'connected_type' => 'boatparty_to_venue',
													'connected_items' => get_queried_object(),
													'nopaging' => true,
												) 
											);
